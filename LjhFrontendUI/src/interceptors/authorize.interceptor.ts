...
export class HttpInterceptorService implements HttpInterceptor {
  ...
  private handle401Error(
    ...
    let dataToSend = {
      accessToken: localStorage.getItem('localStorage_jwt'),
      refreshToken: localStorage.getItem('localStorage_refreshToken'),
    };
    ...
    return this.userService.getRefreshToken(dataToSend).pipe(
      switchMap((response: any) => {
        ...
        if (newToken && newRefreshToken) {
          localStorage.setItem('localStorage_jwt', newToken);
          localStorage.setItem('localStorage_refreshToken', newRefreshToken);
